...
1. `schema.sql` を Supabase の SQL Editor に貼って実行
   ...
   - これで `profiles / organizations / org_members / invitations / projects / project_assignees / org_inventory / org_remnants / cut_plans / org_weight_calcs / org_custom_materials / org_custom_stock_lengths` が作られ、RLS ポリシーと triggers / RPC も入る
...
Phase A の事業所共有テーブルは、既存の device_id テーブルと衝突しないよう `org_inventory` / `org_remnants` / `org_custom_materials` / `org_custom_stock_lengths` を使う。
