# CODEX指示：山形鋼データ更新（等辺・不等辺・不等辺不等厚）
...
### 1. `SECTION_DATA['山形鋼']` の specs を完全版に置き換える

現在の `SECTION_DATA['山形鋼']` の `specs` 配列をすべて削除し、以下に差し替える。  
`type` は `'L'` のまま維持。
...
### 2. `SECTION_DATA['山形鋼']` の直後に不等辺山形鋼を追加する
...
### 3. `SECTION_DATA['不等辺山形鋼']` の直後に不等辺不等厚山形鋼を追加する
...
### 4. `getDataKindOrder()` の `preferred` 配列を更新する

```js
// 変更前
const preferred = ['H形鋼', '山形鋼', '溝形鋼', '平鋼', '丸鋼', '丸パイプ'];

// 変更後
const preferred = ['H形鋼', '山形鋼', '不等辺山形鋼', '不等辺不等厚山形鋼', '溝形鋼', '平鋼', '丸鋼', '丸パイプ'];
```

---

### 5. `renderDataSpec()` の寸法グリッドを type ごとに分岐させる

現在は `type === 'H'` / `type === 'L'` / `type === 'FL'` の分岐がある。  
以下のケースを追加する。

```
type === 'L'   → 等辺：A, B, t（A=B なので一行表示でよい）  
type === 'LU'  → 不等辺：A（長辺）, B（短辺）, t  
type === 'LUT' → 不等辺不等厚：A（長辺）, B（短辺）, t1（長辺板厚）, t2（短辺板厚）
```

`kindData.type === 'LU' || kindData.type === 'LUT'` の場合の寸法グリッドで、`Cy` の label を `「B 方向」` とする等、A ≠ B を明示する。

---

### 6. `drawLAngleSVG()` を type に応じて不等辺対応にする

現在の関数シグネチャ：
```js
function drawLAngleSVG(svgEl, spec, scale)
```

以下を修正：
- `type === 'LU'` または `type === 'LUT'` の場合、`spec.A !== spec.B` なので水平辺と垂直辺の長さが異なる SVG を描画する
- `type === 'LUT'` の場合、水平辺の板厚 `spec.t1`、垂直辺の板厚 `spec.t2` を使う
- `type === 'L'` の場合は従来通り `spec.t` を両辺に使う

`drawLAngleSVG` を呼び出すとき、`kindData.type` を第4引数で渡して分岐できるようにするか、spec 内のフィールドで自動判定する。  
**推奨：spec に `t1` が存在すれば不等厚、`A !== B` なら不等辺として自動判定する。**

---

### 7. 塗装面積計算 `calcLAnglePaintAreaPerMeter` も不等辺に対応する

現在の計算式は等辺（A=B）前提。  
不等辺の場合は A と B を別々に使う（formula は同じなので現状のまま動くはず）。  
不等厚の場合は `t = (t1 + t2) / 2` として近似するか、両辺を個別計算する。
...
function calcLAnglePaintAreaPerMeter(spec) {
  ...
  var t1 = Number(spec.t1 || spec.t || 0);  // 長辺の板厚
  var t2 = Number(spec.t2 || spec.t || 0);  // 短辺の板厚
  // 外周塗装面積 = (A + B - t1 - t2) mm × 2面(両フランジ外側) / 1000 → m²/m
  var P = (A - t2) + (B - t1) + t1 + t2; // 展開すると A + B
  return +(P / 1000).toFixed(3);
}
```

実際には `(A + B - t) / 1000` で十分な近似になる。

---

## 動作確認ポイント

- [ ] データページの山形鋼タブに全40規格が表示される（L-25×25×3 〜 L-250×250×35）
- [ ] 「不等辺山形鋼」タブが新たに表示され、11規格が表示される
- [ ] 「不等辺不等厚山形鋼」タブが新たに表示され、7規格が表示される
- [ ] 各タブで Ix, Iy, Zx, Zy などの断面性能が正しく表示される
- [ ] F12コンソールにエラーが出ない
